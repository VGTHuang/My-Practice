...
int gamescore = 0;
enum keyboard{ROTATE, DROP, LEFT, RIGHT, EXIT};      // corresponding to w, s, a, d, x
...
public:
	...
	int conflict();   //return int type: if not conflict, return 0;
	                  //conflict with left wall, return 1;
	                  //conflict with right wall, return 2;
					  //error, return 3;
	                  //conflict with bottom or static blocks, return -1
	void rotate();
...
void Tetromino::moveHorizontal(bool left){     //horizontal move, until conflict with either walls
	if(left){
		x--;
		if (conflict())
			x++;
...
void Logic(Tetromino& tet){
	...
	case ROTATE:
		tet.rotate();
		break;
	case DROP:               //when a tet is dropped, then it becomes static, and memory space is reused for next tet
		tet.moveDrop();
		tet.pasteToCanvas();
		tet.type = rand() % 7 + 1;
		tet.initTet();
...
int main(){
	...
	while(!game_over && !game_win){
		...
		CheckOver();   //have it reached the top?
	}
	system("cls");
